...
from utils.utils import get_model_tokenizer, process_candidate
...
def get_data(args):
    ...
    """
    data_path = f"{args.root_path}/results/{args.decomp_data}/decomp/test_demo_rank.json"
    dataset_dict = {"prompt":[], "chosen":[], "rejected":[]}
    with open(data_path) as dataset_file:
        for line in dataset_file:
            this_item = json.loads(line)
            prompt, decomps, qualify_cands, ranks = this_item["question"], this_item["decomposition"], this_item["qualified decomp"], this_item["rank"]
            qualify_cands = process_candidate(qualify_cands)
            for i in range(len(decomps)-1):
                this_r = ranks[i]
                this_decomp = decomps[i]
                if len(this_decomp.split("<s>")) > 1:
                    for j in range(i+1, len(decomps)):
                        next_r = ranks[j]
                        next_decomp = decomps[j]
                        if len(next_decomp.split("<s>")) > 1:
                            if next_r < this_r:
                                if j in qualify_cands:
                                    dataset_dict['prompt'].append(prompt)
                                    dataset_dict["chosen"].append(next_decomp)
                                    dataset_dict["rejected"].append(this_decomp)
                            elif this_r < next_r:
                                if i in qualify_cands:
                                    dataset_dict['prompt'].append(prompt)
                                    dataset_dict["chosen"].append(this_decomp)
                                    dataset_dict["rejected"].append(next_decomp)
...
if __name__ == "__main__":
    ...
    for name, param in model.named_parameters():
        if param.requires_grad and name == "base_model.model.lm_head.weight":
            param.requires_grad = False
    _, model_ref = get_model_tokenizer(args.base_model, args)
    ...
    training_args = TrainingArguments(
        ...
        # eval_steps=500,
        output_dir=f"{args.root_path}/save_models/dpo/{model_name}",
        # optim="rmsprop",
        # warmup_steps=150,
    ...
    # 6. train
    dpo_trainer.train()
    model_dir = f"{args.root_path}/save_models/dpo/{model_name}/final"
